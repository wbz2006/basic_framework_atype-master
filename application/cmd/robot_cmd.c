...
void RobotCMDInit()
{
    ...
    vision_recv_data = VisionInit(&huart8); // 视觉通信串口
...
static void MouseKeySet()
{
    ...
    chassis_cmd_send.vy = rc_data[TEMP].key[KEY_PRESS].d * 300 - rc_data[TEMP].key[KEY_PRESS].a * 300;
...
static void EmergencyHandler()
{
    ...
    // 遥控器右侧开关为[上],恢复正常运行
    if (switch_is_up(rc_data[TEMP].rc.switch_right) || switch_is_mid(rc_data[TEMP].rc.switch_right) || switch_is_down(rc_data[TEMP].rc.switch_right) )
    {
